...
export const doApprove = createAsyncThunk('GET/APRROVE',async(payload,{rejectWithValue})=>{
    ...
    try {
        result = await axios.get("https://dangoon.duckdns.org/requestpurchase/approve", {
          params: {
             b_id: payload.b_id,
             r_id:payload.r_id,
          }
...
export const doCancel = createAsyncThunk('DELETE/CANCEL',async(payload,{rejectWithValue})=>{
    ...
    try {
        result = await axios.delete("https://dangoon.duckdns.org/requestpurchase/cancel",{
        params:{
             b_id: payload.b_id,
             r_id:payload.r_id,
        }
...
export const doComplete = createAsyncThunk('GET/COMPELETE',async(payload,{rejectWithValue})=>{
    ...
    try {
        result = await axios.put("https://dangoon.duckdns.org/requestpurchase/complete", {
               b_id: payload.b_id,
               r_id:payload.r_id,
        });
      } catch (err) {
